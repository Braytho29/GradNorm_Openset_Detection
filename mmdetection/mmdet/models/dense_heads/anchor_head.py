...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def get_bboxes(self,
        ...
        if with_nms:
            ...
            result_list = self._get_bboxes(mlvl_cls_scores, mlvl_bbox_preds,
                                           ...
                                           scale_factors, cfg, rescale,True, cls_scores)
        else:
            result_list = self._get_bboxes(mlvl_cls_scores, mlvl_bbox_preds,
                                           mlvl_anchors, img_shapes,
    ...
    def _get_bboxes(self,
        ...
        mlvl_bboxes = []
        mlvl_scores = []
        mlvl_scores_attached = []
        for cls_score, bbox_pred, anchors, cls_scores_attached in zip(mlvl_cls_scores,
                                                 mlvl_bbox_preds,
                                                 mlvl_anchors,
                                                 cls_scores):
            ...
            cls_scores_attached = cls_scores_attached.permute(0, 2, 3,
                                                              1).reshape(batch_size, -1,
                                                                         self.cls_out_channels)
            if self.use_sigmoid_cls:
                scores = cls_score.sigmoid()
                scores_attached = cls_scores_attached.sigmoid()
            else:
                scores = cls_score.softmax(-1)
                scores_attached = cls_scores_attached.softmax(-1)
            ...
            if nms_pre > 0:
                ...
                scores_attached = scores_attached[batch_inds, topk_inds, :]

            bboxes = self.bbox_coder.decode(
                anchors, bbox_pred, max_shape=img_shapes)
            mlvl_bboxes.append(bboxes)
            mlvl_scores.append(scores)
            mlvl_scores_attached.append(scores_attached)
        ...
        batch_mlvl_scores_attached = torch.cat(mlvl_scores_attached, dim=1)
        ...
        if torch.onnx.is_in_onnx_export() and with_nms:
            ...
            if not self.use_sigmoid_cls:
                ...
                batch_mlvl_scores_attached = batch_mlvl_scores_attached[..., :num_classes]
        ...
        if self.use_sigmoid_cls:
            ...
            batch_mlvl_scores_attached = torch.cat([batch_mlvl_scores_attached, padding], dim=-1)
        
        if with_nms:
            det_results = []
            for (mlvl_bboxes, mlvl_scores, mlvl_scores_attached) in zip(batch_mlvl_bboxes,
                                                                        batch_mlvl_scores,
                                                                        batch_mlvl_scores_attached):
                det_bbox, det_label = multiclass_nms(mlvl_bboxes, mlvl_scores,
                                                     cfg.score_thr, cfg.nms,
                                                     cfg.max_per_img, return_logits = self.return_logits,
                                                     multi_logits = batch_mlvl_scores_attached)
